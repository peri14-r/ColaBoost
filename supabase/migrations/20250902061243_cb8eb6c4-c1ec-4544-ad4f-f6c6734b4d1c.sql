-- CollabSpace Complete Database Schema Migration
...
CREATE INDEX IF NOT EXISTS idx_profiles_search ON public.profiles USING GIN(to_tsvector('english', display_name || ' ' || COALESCE(bio, '') || ' ' || COALESCE(array_to_string(skills, ' '), '')));
...
CREATE INDEX IF NOT EXISTS idx_collab_requests_search ON public.collab_requests USING GIN(to_tsvector('english', title || ' ' || COALESCE(description, '') || ' ' || COALESCE(array_to_string(tags, ' '), '')));
